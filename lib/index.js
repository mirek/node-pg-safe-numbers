...
function safeParseInt(text) {
  var unsafeHandler = arguments.length <= 1 || arguments[1] === undefined ? defaultUnsafeInt : arguments[1];
...
function floatCompare(parsed, text) {
  var precision = (text.match(/[0-9]/g) || []).length - (Math.abs(parsed) < 1 && parsed !== 0 ? 1 : 0);
...
function safeParseFloat(text) {
  var unsafeHandler = arguments.length <= 1 || arguments[1] === undefined ? defaultUnsafeFloat : arguments[1];
...
function pgSetTypeParsers() {
  ...
  var unsafeInt = _ref$unsafeInt === undefined ? defaultUnsafeInt : _ref$unsafeInt;
  var _ref$unsafeFloat = _ref.unsafeFloat;
  var unsafeFloat = _ref$unsafeFloat === undefined ? defaultUnsafeFloat : _ref$unsafeFloat;

  var PgTypes = { Int8: 20, Numeric: 1700 };

  var pg = pg_ ? pg_ : require('pg');
